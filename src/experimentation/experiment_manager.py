...
class ExperimentManager:
    ...
    def run_experiments(self):
        ...
        for subset_name, features in self.config["feature_subsets"].items():
            ...
            X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=self.config["random_seed"])
            ...
            for model_name in self.config["models_to_evaluate"]:
                try:
                    ...
                    best_pipeline, best_params = optimizer.optimize(model_name, X_train, y_train)
                    
                    # 4. Evaluation
                    # Cross validation on full training set
                    cv_metrics = self.evaluator.cross_validate_pipeline(best_pipeline, X_train, y_train)
                    
                    # Standalone test evaluation
                    test_metrics, _ = self.evaluator.evaluate_pipeline(best_pipeline, X_train, y_train, X_test, y_test)
                    ...
                    if importances is not None:
                        ...
                        try:
                            ...
                            cat_transformer = best_pipeline.named_steps["preprocessor"].named_transformers_.get("categorical")
                            if cat_transformer is not None and hasattr(cat_transformer, "get_feature_names_out"):
                                ...
                                cat_names = []
                            num_names = [c for c in features if c not in CATEGORICAL_FEATURES]
                            feature_names = cat_names + num_names
                            
                            if len(feature_names) == len(importances):
                                metadata["feature_importances"] = dict(zip(feature_names, importances.tolist()))
                        except Exception as e:
                            logger.warning(f"Failed to map feature importances: {e}")
                    
                    # Save candidate
                    candidate_path = self.registry.save_candidate(self.experiment_id, f"{model_name}_{subset_name}", best_pipeline, metadata)
                    ...
                    logger.error(f"Failed to evaluate {model_name} on {subset_name}: {e}")
    ...
    def _generate_leaderboard(self):
        ...
        df_results = df_results.sort_values(by="CV_R2_Mean", ascending=False).reset_index(drop=True)
    ...
    def _promote_best_model(self):
        ...
        logger.info(f"Selecting best model: {best_run['Model']} ({best_run['Features']}) with CV R2: {best_run['CV_R2_Mean']:.4f}")
        
        self.registry.promote_to_production(
            candidate_path=best_run["Path"],
            metadata=best_run["Metadata"]
        )
