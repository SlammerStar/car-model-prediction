...
class DataValidator:
    ...
    def _check_missing_values(self, df: pd.DataFrame, features: list):
        ...
        if not missing.empty:
            raise ValueError(
                f"Data validation failed: Missing values detected in features:\n{missing}"
            )

    def _check_target_leakage(self, df: pd.DataFrame, features: list):
        if TARGET_COLUMN in features or "selling_price" in features:
            raise ValueError(
                f"Target leakage detected! Target columns found in feature set: {features}"
            )
    ...
    def _check_correlation(self, df: pd.DataFrame, features: list):
        ...
        if high_corr:
            logger.warning(
                f"Highly correlated features detected (>{self.correlation_threshold}): {high_corr}"
            )
