...
class HyperparameterOptimizer:
    ...
    """
    def __init__(self, preprocessor: ColumnTransformer, n_trials: int = 20, timeout: int = 600, n_jobs: int = 1):
    ...
    def optimize(self, model_name: str, X: pd.DataFrame, y: pd.Series) -> Tuple[Pipeline, Dict[str, Any]]:
        ...
        def objective(trial):
            ...
            if len(X) > 10000:
                idx = np.random.RandomState(RANDOM_STATE).choice(X.index, 10000, replace=False)
            ...
            scores = cross_val_score(pipeline, X_search, y_search, cv=cv, scoring='neg_mean_absolute_error', n_jobs=self.n_jobs)
        ...
        logger.info(f"Optimization finished for {model_name}. Best params: {best_params}")
