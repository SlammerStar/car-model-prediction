...
class VehicleKnowledgeEngine:
    ...
    def _build_knowledge_base(self, df: pd.DataFrame):
        ...
        required_cols = ['brand', 'base_model', 'variant', 'year', 'fuelType', 'transmission']
        ...
        self.kb = df.groupby(required_cols, dropna=False).agg(
    ...
    def get_fuel_types(self, brand: str, base_model: str, year: int, variant: str) -> List[str]:
    ...
    def get_transmissions(self, brand: str, base_model: str, year: int, variant: str, fuel: str) -> List[str]:
    ...
    def get_specs(self, brand: str, base_model: str, year: int, variant: str, fuel: str, transmission: str) -> Optional[Dict[str, Any]]:
        """
        ...
        return {
            'engineSize': round(row['engineSize'], 1) if pd.notna(row['engineSize']) else 0.0,
            'max_power_bhp': round(row['max_power_bhp'], 1) if pd.notna(row['max_power_bhp']) else 0.0,
    ...
    def get_variant_label(self, brand: str, base_model: str, year: int, variant: str) -> str:
        """
        ...
        row = subset.iloc[0]
        engine = f"{round(row['engineSize'], 1)}L " if pd.notna(row['engineSize']) else ""
        fuel = str(row['fuelType'])
        trans = str(row['transmission'])
        
        return f"{variant} ({engine}{fuel} {trans})"

    def get_similar_configurations(self, brand: str, base_model: str, year: int, variant: str) -> List[Dict[str, Any]]:
        """
