...
def get_brand_image(brand):
    ...
    return BRAND_CAR_IMAGES.get(brand, "https://images.unsplash.com/photo-1552519507-da3b142c6e3d?q=80&w=800&auto=format&fit=crop")
...
if page == "Predict Price":
    ...
    with r1c2:
        year = st.slider("Manufacturing Year", min_value=1996, max_value=int(CURRENT_YEAR), value=2021)
    with r1c3:
        mpg = st.number_input("Fuel Efficiency (km/l)", min_value=0.0, max_value=100.0, value=float(df["mpg"].median()), step=0.5)
    ...
    with r3c2:
        mileage = st.number_input("Mileage (kms)", min_value=0, max_value=500000, value=30000, step=1000)
    with r3c3:
        ...
        with st.spinner("Analyzing market data..."):
            try:
                result = predict_price(
                    ...
                    fuel_type=fuel_type, mpg=mpg, engine_size=engine_size,
                    pipeline=pipeline
                )
                ...
                with res1:
                    ...
                    conf_color = "#00FF88" if conf_val >= 85 else ("#FFC857" if conf_val >= 75 else "#FF3B30")
                ...
                with res2:
                    ...
                    fig_trend.add_trace(go.Scatter(
                        x=months, y=trend_y, mode='lines',
                        line=dict(color='#00FF88', width=2.5, shape='spline', smoothing=1.3),
                        fill='tozeroy',
...
# ===========================================================================
    ...
    if metadata:
        ...
        with kpi2:
            st.metric(label="R-Squared Accuracy", value=f"{tuned.get('R2_Score', 0) * 100:.1f}%")
        with kpi3:
            st.metric(label="Mean Absolute Error", value=f"INR {tuned.get('MAE', 0):,.0f}")
        ...
        if not comparison.empty:
            ...
            display_comp["R2_Score"] = display_comp["R2_Score"].apply(lambda x: f"{x*100:.2f}%" if x > 0 else "N/A")
            display_comp["MAE"] = display_comp["MAE"].apply(lambda x: f"INR {x:,.0f}" if x < 1e10 else "N/A")
            display_comp["RMSE"] = display_comp["RMSE"].apply(lambda x: f"INR {x:,.0f}" if x < 1e10 else "N/A")
        ...
        with col_img1:
            st.markdown(f'<div class="section-title" style="font-size: 0.88rem;">{icon("target", 14, "#00A3FF")} Actual vs Predicted</div>', unsafe_allow_html=True)
        ...
        with col_img2:
            st.markdown(f'<div class="section-title" style="font-size: 0.88rem;">{icon("bar-chart", 14, "#00A3FF")} Residual Distribution</div>', unsafe_allow_html=True)
        ...
        st.markdown(f'<div class="section-title" style="font-size: 0.88rem; margin-top: 12px;">{icon("layers", 14, "#00A3FF")} SHAP Feature Importance</div>', unsafe_allow_html=True)
        img_path = IMAGES_DIR / "shap_summary.png"
        if img_path.exists():
            st.image(str(img_path), use_container_width=True)
        else:
            st.info("SHAP summary not available. Run training to generate visualizations.")
    else:
...
# ===========================================================================
    ...
    with col_chart1:
        avg_price = df.groupby("brand")["price_inr"].mean().reset_index().sort_values("price_inr", ascending=False)
        fig = px.bar(avg_price, x="brand", y="price_inr", title="Average Valuation by Brand",
                     color="price_inr", color_continuous_scale=[[0, "#0077FF"], [1, "#00A3FF"]], template="plotly_dark")
        fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_family="Inter",
                          font_color="#94A3B8", title_font=dict(family="Montserrat", size=14, color="#E5E7EB"),
                          xaxis=dict(gridcolor="rgba(255,255,255,0.03)", title=""),
                          yaxis=dict(gridcolor="rgba(255,255,255,0.03)", title="Price (INR)"),
                          coloraxis_showscale=False, margin=dict(t=40, b=20), height=340)
        st.plotly_chart(fig, use_container_width=True)

    with col_chart2:
        fuel_dist = df["fuelType"].value_counts().reset_index()
        fig2 = px.pie(fuel_dist, names='fuelType', values='count', title="Fuel Type Market Share",
                      color_discrete_sequence=["#00A3FF", "#0077FF", "#1E3A5F", "#94A3B8"],
                      template="plotly_dark", hole=0.55)
        fig2.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_family="Inter",
                           font_color="#94A3B8", title_font=dict(family="Montserrat", size=14, color="#E5E7EB"),
                           margin=dict(t=40, b=20), height=340, legend=dict(font=dict(size=10)))
        fig2.update_traces(textinfo='percent+label', textfont_size=10)
        st.plotly_chart(fig2, use_container_width=True)

    sample_size = min(2000, len(df))
    fig3 = px.scatter(df.sample(sample_size), x="mileage", y="price_inr", color="brand",
                      title=f"Mileage vs Valuation (Sample {sample_size:,})", template="plotly_dark", opacity=0.55,
                      color_discrete_sequence=px.colors.qualitative.Set2)
    fig3.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_family="Inter",
                       font_color="#94A3B8", title_font=dict(family="Montserrat", size=14, color="#E5E7EB"),
                       xaxis=dict(gridcolor="rgba(255,255,255,0.03)", title="Mileage"),
                       yaxis=dict(gridcolor="rgba(255,255,255,0.03)", title="Price (INR)"),
                       margin=dict(t=40, b=20), height=400, legend=dict(font=dict(size=10)))
...
# ===========================================================================
    ...
    with filter1:
        selected_brand = st.selectbox("Filter by Brand", ["All Brands"] + sorted(df["brand"].unique().tolist()))
    with filter2:
        selected_fuel = st.selectbox("Filter by Fuel Type", ["All"] + sorted(df["fuelType"].unique().tolist()))
    with filter3:
        selected_trans = st.selectbox("Filter by Transmission", ["All"] + sorted(df["transmission"].unique().tolist()))
    ...
    with stat2:
        if len(display_df) > 0:
            st.metric("Price Range", f"{format_price_inr(display_df['price_inr'].min())} - {format_price_inr(display_df['price_inr'].max())}")
        else:
            st.metric("Price Range", "N/A")
    with stat3:
        st.metric("Avg Mileage", f"{display_df['mileage'].mean():,.0f} kms" if len(display_df) > 0 else "N/A")

    st.markdown("<div style='height: 6px;'></div>", unsafe_allow_html=True)

    styled_df = display_df.copy()
    styled_df["Price"] = styled_df["price_inr"].apply(format_price_inr)
    styled_df = styled_df[["brand", "model", "year", "Price", "transmission", "fuelType", "mileage", "engineSize"]]
    styled_df.columns = ["Brand", "Model", "Year", "Price", "Transmission", "Fuel Type", "Mileage", "Engine (L)"]

    st.dataframe(styled_df.head(100), use_container_width=True, hide_index=True)
    st.markdown(f"<div style='color: #94A3B8; text-align: right; font-size: 0.75rem; margin-top: 6px;'>Showing top 100 of {len(display_df):,} vehicles</div>", unsafe_allow_html=True)
...
# ===========================================================================
    ...
    selected_brand = st.selectbox("Filter by Brand", ["All Brands"] + sorted(df["brand"].unique().tolist()), key="data_brand")
    display_df = df if selected_brand == "All Brands" else df[df["brand"] == selected_brand]
    ...
    with dist1:
        ...
        fig_year = px.bar(year_dist, x="Year", y="Count", title="Distribution by Year",
                          template="plotly_dark", color_discrete_sequence=["#00A3FF"])
    ...
    with dist2:
        fig_price = px.histogram(display_df, x="price_inr", nbins=40, title="Price Distribution",
                                 template="plotly_dark", color_discrete_sequence=["#0077FF"])
...
# ===========================================================================
    ...
    features = [
        ("cpu", "ML Pipeline", "Automated preprocessing with scikit-learn ColumnTransformer, feature engineering, and hyperparameter-tuned Random Forest models."),
        ("target", "Price Prediction", "Accurate used car valuations with confidence intervals, depreciation analysis, and Indian market calibration."),
        ("layers", "Recommendations", "K-Nearest Neighbors recommender system suggests similar vehicles based on transformed feature space."),
        ("bar-chart", "Model Analytics", "Compare 5 algorithms side-by-side with R-squared, MAE, RMSE metrics and SHAP explainability plots."),
        ("globe", "Market Insights", "Interactive charts covering brand valuations, fuel type distributions, and mileage-price correlations."),
        ("shield-check", "Data Quality", f"Trained on {metadata.get('n_training_samples', 66036):,} samples across 8 brands with robust cross-validation."),
    ]
    ...
    st.markdown(f'<div class="section-title" style="margin-top: 10px;">{icon("zap", 16, "#00A3FF")} Technology Stack</div>', unsafe_allow_html=True)
